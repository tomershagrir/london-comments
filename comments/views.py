...
def post_comment(request, operator=None):
    try:
        ...
        comment = Comment.query().create(owner = owner, body=body, author = author, operator = operator)
        return JsonResponse('{"status":"ok", "pk":"%s", "author":"%s", "operator":"%s", "body":"%s", "created":"%s"}' %
                            (comment['pk'], comment['author'] or "Anonymous", comment['operator'], comment['body'], comment['created'].strftime('%Y-%m-%d, %H:%M')))
